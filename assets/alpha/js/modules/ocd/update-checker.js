...
const checkError = (element, title, badgeEl) => {
  ...
  element.title = title;
  badgeEl.classList.add("ocd-check-error");
};
...
const compareAlphaVersion = (remoteVersion) => {
  if (remoteVersion && remoteVersion !== localAlphaVersion) {
    ...
    appendTemplate(
      document.querySelector('[data-alpha="ocd-alpha-updates-template"]'),
    );
  } else if (remoteVersion) {
    localAlphaVersionEl.title = "You have the latest version";
    alphaBadge.classList.add("ocd-no-update");
    appendTemplate(
      document.querySelector('[data-alpha="ocd-no-alpha-updates-template"]'),
    );
  } else {
    checkError(
      localAlphaVersionEl,
...
const compareHugoVersion = (remoteVersion) => {
  if (remoteVersion && remoteVersion !== localHugoVersion) {
    ...
    appendTemplate(
      document.querySelector('[data-alpha="ocd-hugo-updates-template"]'),
    );
  } else if (remoteVersion) {
    localHugoVersionEl.title = "You have the latest version";
    hugoBadge.classList.add("ocd-no-update");
    appendTemplate(
      document.querySelector('[data-alpha="ocd-no-hugo-updates-template"]'),
    );
  } else {
    checkError(
      localHugoVersionEl,
